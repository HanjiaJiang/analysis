...
T = 20.
T_start = 0.  # starting point of analysis
...
Pop_sizes = [pop_sizes[i:i + 2]
             for i in np.arange(0, len(pop_sizes), 2)]
print('Population sizes:')
print(Pop_sizes)
print()

Raw_first_gids = [raw_first_gids[i:i + 2] for i in
                  np.arange(0, len(raw_first_gids), 2)]

First_gids = [first_gids[i:i + 2] for i in np.arange(0, len(first_gids), 2)]

Last_gids = [last_gids[i:i + 2] for i in np.arange(0, len(last_gids), 2)]
...
for i in np.arange(num_pops):
    for j in np.arange(first_gids[i], last_gids[i]):
        spikes = [spk for spk in neuron_spikes[j] if (spk > T_start and spk <= T)]
        temp += len(spikes)
