...
def create_individual():
    new_map = origin_map + np.multiply(0.5*origin_map, np.random.randn(13, 13))
    return new_map
...
def mutSNP1(ind, p):
    ...
    for i, row in enumerate(new):
        for j, item in enumerate(row):
            if np.random.random() <= p:
                new[i][j] = item + 0.5*item*(np.random.randn())
    return type(ind)(new)
    ...
    if on_server:
        ...
        for i in range(len(survivors)):
            ...
            if os.path.isdir(datapath) is False:
                os.mkdir(datapath)
            np.save(datapath + 'coef_arr.npy', 0.1*np.random.randn(4,4) + 0.1)
...
while min(fits) > 0 and g < max_generations:
    ...
    np.save('fitness_evolved_g{}.npy'.format(g), fitness_evolved)
    g += 1
